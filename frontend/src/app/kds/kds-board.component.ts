...
type TicketAction = { status: KitchenTicketStatus; label: string; intent: 'primary' | 'secondary' };
...
export class KdsBoardComponent implements OnInit {
  ...
  protected actionsFor(ticket: KitchenTicketViewModel): TicketAction[] {
    switch (ticket.status) {
      case 'Received':
        return [{ status: 'InPreparation', label: 'Start preparing', intent: 'primary' }];
      case 'InPreparation':
        return [{ status: 'Ready', label: 'Mark ready', intent: 'primary' }];
      case 'Ready':
        return [{ status: 'Completed', label: 'Mark picked up', intent: 'secondary' }];
      default:
        return [];
    }
  }
  ...
  protected actionClass(action: TicketAction): string {
    return `ticket__action ticket__action--${action.intent}`;
  }

  protected isUpdatingTicket(ticket: KitchenTicketViewModel): boolean {
    return this.kitchenDisplay.isUpdating(ticket.orderId);
  }

  protected advanceTicket(ticket: KitchenTicketViewModel, action: TicketAction): void {
    void this.kitchenDisplay.updateTicketStatus(ticket.orderId, action.status);
  }
