...
export class KitchenDisplayService {
  ...
  private readonly updatingSignal = signal<Set<number>>(new Set());
  ...
  isUpdating(orderId: number): boolean {
    return this.updatingSignal().has(orderId);
  }

  async updateTicketStatus(orderId: number, status: KitchenTicketStatus): Promise<boolean> {
    if (this.isUpdating(orderId)) {
      return false;
    }

    this.setUpdating(orderId, true);
    try {
      const url = this.createUrl(`/orders/${orderId}`);
      await firstValueFrom(this.http.patch<void>(url, { status }));
      this.applyLocalStatus(orderId, status);
      this.errorSignal.set(null);
      return true;
    } catch (error) {
      console.error('Unable to update kitchen ticket', error);
      this.errorSignal.set('Unable to update ticket status. Please retry.');
      return false;
    } finally {
      this.setUpdating(orderId, false);
    }
  }
  ...
  private applyLocalStatus(orderId: number, status: KitchenTicketStatus): void {
    const current = this.ticketMap.get(orderId);
    if (!current) {
      return;
    }

    if (!this.shouldDisplay(status)) {
      this.removeTicket(orderId);
      return;
    }

    const updated: KitchenTicketViewModel = { ...current, status };
    this.ticketMap.set(orderId, updated);
    this.publishTickets();

    if (status === 'Ready' && current.status !== 'Ready') {
      this.readySubject.next(updated);
    }
  }
  ...
  private shouldDisplay(status: KitchenTicketStatus): boolean {
    return status !== 'Completed' && status !== 'Cancelled';
  }

  private setUpdating(orderId: number, updating: boolean): void {
    this.updatingSignal.update((current) => {
